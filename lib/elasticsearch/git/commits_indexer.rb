module Elasticsearch
  module Git
    module CommitsIndexer
      ...
      def index_commits(client, index_name, repository_id, repository_for_indexing, logger, from_rev: nil, to_rev: nil)
    ...
    private
      ...
      def index_commit_operation(commit, repository_id, index_name)
        {
          index:  {
            _index: index_name, _type: "repository", _id: "#{repository_id}_#{commit.oid}",
            data: {
              commit: {
                type: "commit",
                rid: repository_id,
